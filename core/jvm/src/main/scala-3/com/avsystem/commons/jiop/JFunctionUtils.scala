...
trait JFunctionUtils {
  ...
  inline def jBooleanSupplier(inline expr: => Boolean): JBooleanSupplier = () => expr
  ...
  inline def jDoubleSupplier(inline expr: => Double): JDoubleSupplier = () => expr
  ...
  inline def jIntSupplier(inline expr: => Int): JIntSupplier = () => expr
  ...
  inline def jLongSupplier(inline expr: => Long): JLongSupplier = () => expr
  ...
  inline def jObjDoubleConsumer[T](inline code: (T, Double) => Any): JObjDoubleConsumer[T] = (t, d) => {
    code(t, d)
    ()
  }
  ...
  inline def jSupplier[T](inline expr: => T): JSupplier[T] = () => expr

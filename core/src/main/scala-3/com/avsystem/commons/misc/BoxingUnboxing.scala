...
object Boxing extends BoxingCompat with LowPrioBoxing {
...
trait LowPrioBoxing extends LowPrioBoxingCompat { this: Boxing.type =>
  given [A] => Boxing[A, A] = Boxing(identity)
}

case class Unboxing[+A, -B](fun: B => A) extends AnyVal
object Unboxing extends UnboxingCompat with LowPrioUnboxing {
...
trait LowPrioUnboxing extends LowPrioUnboxingCompat { this: Unboxing.type =>

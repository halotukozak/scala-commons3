      ...
      case untpd.Import(expr, selectors) =>
        expr match {
          ...
          case untpd.Ident(name) if name.toString == "java" =>
            selectors.foreach { case untpd.ImportSelector(imported: untpd.Ident, renamed, _) =>
              if (imported.name.toString == "util" && renamed.isEmpty)
                emitReport(
                  tree.srcPos,
          ...
          case untpd.Select(qual: untpd.Ident, name) if name.toString == "util" && qual.name.toString == "java" =>
            selectors.foreach { case untpd.ImportSelector(imported, renamed, _) =>
              if (renamed.isEmpty) {
                emitReport(
                  tree.srcPos,
                  "Don't import java.util: either import with rename (e.g. import java.{util => ju}) " +
